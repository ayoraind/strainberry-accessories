...
def main( argv = None ):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for ref_id in refSeqDict:
            # nucmer
            nucmer_cmd=['nucmer','--maxmatch','-p',os.path.join(ref_id),refSeqDict[ref_id],opt.fastaFile]
            ...
            if nucmer != 0:
                ...
                return 2
            # delta-filter
            with open(os.path.join(cwd,f'{ref_id}.qdelta'),'w') as deltaout:
                delta_cmd=['delta-filter','-q',os.path.join(cwd,f'{ref_id}.delta')]
                delta=subprocess.call(delta_cmd, stdout=deltaout, stderr=subprocess.DEVNULL)
                if delta != 0:
                    eprint(f'error running command: {" ".join(delta_cmd)}')
                    return 2
            # show-coords
            with open(os.path.join(cwd,f'{ref_id}.qcoords'),'w') as qcoords:
                showcoords_cmd=['show-coords','-qHTlc',os.path.join(cwd,f'{ref_id}.qdelta')]
            ...
            with open(os.path.join(cwd,f'{ref_id}.qcoords'),'r') as qcoords:
                for line in qcoords:
                    line = line.rstrip('\n')
